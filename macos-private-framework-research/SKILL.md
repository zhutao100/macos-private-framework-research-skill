...
3. Extract or open the framework image.
   ...
   Preferred extraction order: `ipsw dyld extract` for single-framework extraction with ObjC/stub symbols; `dyld-shared-cache-extractor` when Xcode's `dsc_extractor.bundle` is needed for newer/beta cache formats; Hopper's dyld cache loader for focused GUI/MCP inspection. Load `references/tooling-workflows.md` for alternatives.
...
5. Triage underspecified declarations.
   ...
   Prioritize methods involving `id`, `void *`, anonymous structs, raw collections, blocks, protocol-qualified delegates, XPC-facing classes, entitlement-controlled flows, and selectors observed in client binaries.
...
7. Infer, validate, and refine signatures.
   ...
   Iterate until hard errors are resolved or the evidence gap is explicit. Use compiler diagnostics, selector-label preservation, method encodings, caller behavior, decompiler hints, public SDK typedefs, and neighboring declarations as constraints. Never silently change selector spelling or method arity.
...
| Single framework extraction | `ipsw dyld extract` via `scripts/extract_dyld_framework.sh` | `dyld-shared-cache-extractor`, Hopper dyld cache loader |
...
- `scripts/extract_dyld_framework.sh`: Extract a named framework from the active dyld shared cache using `ipsw` or `dyld-shared-cache-extractor`.
- `scripts/objc_header_triage.py`: Scan reconstructed headers and rank underspecified Objective-C declarations.
- `scripts/build_motif_context.py`: Build JSON and Markdown prompt bundles for a single candidate signature.
- `scripts/objc_signature_linter.py`: Validate reconstructed Objective-C headers with structural checks and optional `clang -fsyntax-only`.
